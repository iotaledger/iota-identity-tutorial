import { AccountBuilder, Subject, AutoSave, DID, ExplorerUrl, KeyPair, KeyType } from "@iota/identity-wasm/node";
...
async function stronghold() {
  ...
  if (process.argv[2].toLowerCase() == "create-did"){
    if(process.argv.length != 5){
      ...
      console.error("Please use: \"npm run start create-did <name> <stronghold-password>\"");
      return;
    }
  ...
  if (process.argv[2].toLowerCase() == "create-vm"){
    if(process.argv.length != 6){
      ...
      console.error(`Please use: "npm run start create-vm <identity-name> <stronghold-password> <verification-fragment>`)
      return;
    }
    let name = process.argv[3];
    let password = process.argv[4];
    let fragment = process.argv[5];
  ...
  if (process.argv[2].toLowerCase() == "add-revocation-bitmap"){
    if(process.argv.length != 6){
      console.error("Error: Command arguments are incorrect!")
      console.error(`Please use: "npm run start create-revocation-list <identity-name> <stronghold-password> <revocation-fragment>`)
      return;
    }
    let name = process.argv[3];
    let password = process.argv[4];
  ...
  if (process.argv[2].toLowerCase() == "create-vc"){
    ...
    createSignedVerifiableCredential(issuerName, issuerPassword, subjectName, subjectDid, verificationMethodFragment, revocationBitmapFragment, revocationIndex);
  }
  ...
  if (process.argv[2].toLowerCase() == "create-vp"){
    ...
    createVerifiablePresentation(holderName, holderPassword, fragment, challenge);
  }
  ...
  if (process.argv[2].toLowerCase() == "revoke-vc"){
    ...
    revokeVC(issuerName, issuerPassword, revocationBitmapFragment, parseInt(revocationIndex));
  }
}
